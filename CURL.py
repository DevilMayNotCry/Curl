...
def main():
	...
	if(args.regen):
		...
		net = torch.load(args.checkpoint_filepath, map_location=lambda storage, location: storage)
		net.cuda()
		net.eval()

		for batch_num, data in enumerate(inference_data_loader):
			# 加载数据, 送到 GPU
			image_low_quality, image_name = Variable(data['input']).cuda(), data['name']
		...
		for batch_num, (low_quality, high_quality) in enumerate(zip(inference_data_loader, label_data_loader)):
			# 加载数据, 送到 GPU
			low_quality_image, low_image_name = Variable(low_quality['input']).cuda(), low_quality['name']
			high_quality_image, high_image_name = Variable(high_quality['input']).cuda(), high_quality['name']
		...
		for epoch in range(num_epoch):
			...
			for batch_num, data in enumerate(training_data_loader, 0):
				# load images
				input_img_batch, output_img_batch, category = Variable(data['input'], requires_grad=False).cuda(), Variable(data['expertC_gt'], requires_grad=False).cuda(), data['name']
				# forward phase
