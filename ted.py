...
class TED(nn.Module):
    ...
    def __init__(self):
        ...
        self.up_conv1x1_4 = nn.Conv2d(16, 12, 1)
        ...
        self.conv_fuse1 = nn.Conv2d(208, 12, 1)
    ...
    def forward(self, x):
        ...
        # print('dconv_down1  :  {}'.format(x.shape))
        ...
        # print('cat 之前   x  :  {}  |  conv1_fuse  :  {}'.format(x.shape, conv1_fuse.shape))
        x = torch.cat([x, conv1_fuse], dim=1)
        del conv1

        x = self.dconv_up1(x)
        # print('x  :  {}  | x_in_tile  :  {}'.format(x.shape, x_in_tile.shape))
        x = x+x_in_tile
...
class LocalNet(nn.Module):
    ...
    def forward(self, x_in):
        ...
        """
        # print('LocalNet  :  {}'.format(x_in.shape))
        # print('refpad  :  {}'.format(self.refpad(x_in).shape))
        # print('conv1  :  {}'.format(self.conv1(self.refpad(x_in)).shape))
...
class TEDModel(nn.Module):
    ...
    def forward(self, image):
        ...
        """
        # print('TEDModel 这里  image  :  {}'.format(image.shape))
        # image_shuffled = DownSamplingShuffle(image)
        # print('image_shuffled  :  {}'.format(image_shuffled.shape))
        image_shuffled = image
        output_image = self.ted(image_shuffled.float())
        # print('output_image  :  {}'.format(output_image.shape))

        upsampler = SimpleUpsampler(1)
        upsampler = nn.Sequential(*upsampler)
        output_image = upsampler(output_image)
        # print('upsampler 之后  :  {}'.format(output_image.shape))
